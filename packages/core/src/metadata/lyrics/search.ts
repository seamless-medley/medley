...
export async function searchLyrics(artist: string, title: string, helperDomain: string = 'lyrics') {
  ...
  for (const source of sources) {
    const result = await MetadataHelper.for(helperDomain, helper => helper.searchLyrics(artist, title, source.name));
